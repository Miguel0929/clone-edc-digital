class QuizProgramsController < ApplicationController
  ...
  def create
  	...
  	@chapter_content=ChapterContent.new(coursable_id: params[:quiz_id], coursable_type: "Quiz", chapter: @chapter)
      ...
      redirect_to @chapter.program, notice: "Se agrego exitosamente el examen #{@chapter_content.model.name}"
    else
      render :new
    end
  end
