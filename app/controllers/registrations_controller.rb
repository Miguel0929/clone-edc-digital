	...
	def update
		super do
			...
			if current_user.user_detail.nil?
				current_user.create_user_detail(birthdate: old_birthdate, challenge: old_challenge, goal: old_goal)
			else
				current_user.user_detail.update(birthdate: old_birthdate, challenge: old_challenge, goal: old_goal)
			end
		end
	end
