class CertificateNotificationJob < ActiveJob::Base
  ...
  def perform(recipient_address, program, route, certificate_link, certificate)
    ...
    #  if mentor.panel_notifications.complete_mentor.first.nil? || mentor.panel_notifications.complete_mentor.first.status
      	CertificateMailer.certificate_sender(recipient_address, program, route, certificate_link, certificate)
    #  end  
    #end
  end
end  	